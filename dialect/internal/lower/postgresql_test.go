...
func TestPostgreSQLTranslate(t *testing.T) {
	...
	tests := []struct {
		...
		{"P-6_string_agg", "SELECT group_concat(name, ', ') AS \"STRING_AGG(name, ', ')\" FROM t", "SELECT group_concat(name, ', ') AS \"STRING_AGG(name, ', ')\" FROM t"},
...
func TestPostgreSQLSetReturningNameIsNotACall(t *testing.T) {
	...
	for _, input := range tests {
		t.Run(input, func(t *testing.T) {
			t.Parallel()
			got, err := dialect.Translate(dialect.PostgreSQL, input)
			if err != nil {
...
func TestPostgreSQLTranslateUnsupported(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// A construct SQLite cannot express and one the query cannot be
			// read as are both refusals; which of the two a malformed operator
			// is depends on where it stops being readable, and the caller acts
			// on either the same way.
			if !errors.Is(err, dialect.ErrUnsupportedSyntax) && !errors.Is(err, dialect.ErrInvalidSyntax) {
				t.Fatalf("Translate(PostgreSQL, %q) error = %v, want a refusal", tt.input, err)
			}
		})
	}
}
...
func TestPostgreSQLSemanticsMatchTheEngine(t *testing.T) {
	...
	// The clauses have to be written in the order the standard gives them; the
	// other order is not a construct with no SQLite form but a query that
	// cannot be read.
	if _, err := dialect.Translate(dialect.PostgreSQL, `SELECT SUBSTRING(b ESCAPE '#' SIMILAR 'p') FROM t`); !errors.Is(err, dialect.ErrInvalidSyntax) {
		t.Errorf("SUBSTRING with ESCAPE before SIMILAR: error = %v, want ErrInvalidSyntax", err)
	}

	for _, query := range []string{`SELECT chr(0)`, `SELECT chr(-1)`} {
		if _, err := runDialect(t, db, dialect.PostgreSQL, query); err == nil {
