...
func TestGoogleSQLTranslate(t *testing.T) {
	...
	tests := []struct {
		...
		{"G-18_string_agg", "SELECT group_concat(name, ', ') AS \"STRING_AGG(name, ', ')\" FROM t", "SELECT group_concat(name, ', ') AS \"STRING_AGG(name, ', ')\" FROM t"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got, err := dialect.Translate(dialect.GoogleSQL, tt.input)
			if err != nil {
...
func TestGoogleSQLTranslateUnsupported(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			_, err := dialect.Translate(dialect.GoogleSQL, tt.input)
...
func TestGoogleSQLDatePartsBigQueryDefines(t *testing.T) {
	...
	for _, tt := range tests {
		got, err := runDialect(t, db, dialect.GoogleSQL, tt.query)
		if err != nil {
...
func TestGoogleSQLArrayFunctionsAreRejected(t *testing.T) {
	...
	if _, err := dialect.Translate(dialect.GoogleSQL, "SELECT split, farm_fingerprint FROM t"); err != nil {
