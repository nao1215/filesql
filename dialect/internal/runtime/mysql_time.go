...
// This file holds dialects.MySQL's TIME functions, which need a value of their own
// rather than the datetime helpers beside them. A dialects.MySQL TIME is a signed span
...
// mysqlTimeMaxMicros is the largest magnitude a dialects.MySQL TIME holds, 838:59:59.
// A value past it is clamped rather than refused, which is what dialects.MySQL does.
...
// them by. None of these names is a dialects.SQLite keyword or built-in.
...
// clampTime holds a TIME to the range dialects.MySQL stores one in.
func clampTime(micros int64) int64 {
	return max(min(micros, mysqlTimeMaxMicros), -mysqlTimeMaxMicros)
}

// parseMySQLTime reads a dialects.MySQL TIME from a string, accepting the hour-first
...
func parseMySQLTime(s string) (int64, bool) {
	...
	// "T" form is read as well as the space form, which dialects.MySQL does not do -- it
...
// microsecondDigits is how many digits of a second dialects.MySQL keeps.
const microsecondDigits = 6

// parseFractionDigits reads a decimal fraction of a second as microseconds,
// padding or truncating to the six digits dialects.MySQL keeps.
...
// fnMySQLIntervalCompound implements DATE_ADD and DATE_SUB with one of dialects.MySQL's
...
//
// dialects.MySQL separates the fields on any run of punctuation, and a value shorter
...
func fnMySQLIntervalCompound(args []driver.Value) (driver.Value, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: unsupported INTERVAL unit %q", sqlerr.ErrUnsupportedSyntax, unit)
	}
	...
	// A short value names the rightmost fields, which is dialects.MySQL's rule.
...
// is not a digit, which is the delimiter rule dialects.MySQL states: any punctuation may
// separate the fields.
...
func fnMySQLToSeconds(args []driver.Value) (driver.Value, error) {
	...
	// dialects.MySQL counts from the start of year 0, which puts the Unix epoch at day
	// 719528 -- the number TO_DAYS answers for it. The two disagree by one day
	// for a date in year 0 itself, where dialects.MySQL's own documentation says
...
// one. dialects.SQLite has no function of that name, so the call failed by name.
...
// carries, and answers NULL as dialects.MySQL itself does when its zone tables are not
// loaded.
...
// mysqlCompositeParts maps each of dialects.MySQL's compound date-part names to the
...
//
//nolint:gochecknoglobals // a fixed table of dialects.MySQL's own part names
...
// leading one, so the fields concatenate the way dialects.MySQL concatenates them.
//
...
// fraction dialects.MySQL keeps six digits of.
...
// the fraction dropped rather than rounded, which is what dialects.MySQL does.
...
// clamped, both of which are dialects.MySQL's answers. The sign comes from the hour.
...
func addTime(sign int64) scalarFn {
	return func(args []driver.Value) (driver.Value, error) {
		// The second operand is a TIME and not a datetime: dialects.MySQL answers NULL
...
// rather than refusing, because a TIME has no date and dialects.MySQL prints the field
// as if it were empty.
...
// and dialects.MySQL refuses the whole call rather than inventing one.
...
func fnTimeFormat(args []driver.Value) (driver.Value, error) {
	...
	if !ok || format == "" {
		// dialects.MySQL answers NULL for an empty format rather than an empty string.
		return nil, nil
	}
	var b strings.Builder
	// The sign stands in front of the whole result rather than on each field,
	// which is what dialects.MySQL prints: TIME_FORMAT('-01:30:45', '%k %p') is "-1 AM".
	if micros < 0 {
		b.WriteByte('-')
		micros = -micros
	}
...
// know is written as the letter itself, which is what dialects.MySQL does.
...
// below the first is the zero date dialects.MySQL prints, and above the last is past the
// year 9999 it stores.
const (
	minToDays = 366
	maxToDays = 3652424
...
// zeroDate is what dialects.MySQL prints for a day number before the year one.
const zeroDate = "0000-00-00"

// fnToDays implements TO_DAYS(d): the day number of a date, counting from the
// year zero the way dialects.MySQL does.
...
// twoDigitYearPivot is where dialects.MySQL splits a two-digit year: 69 and below are in
...
// maxYear is the last year dialects.MySQL stores a date in.
const maxYear = 9999
...
// way dialects.MySQL refuses it rather than answering.
...
// monthsToPeriod is the inverse of periodToMonths, written the way dialects.MySQL writes
