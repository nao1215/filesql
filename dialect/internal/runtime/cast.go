...
// dialects.SQLite's.
//
// Mapping a cast onto dialects.SQLite's own CAST loses the parts of the operation users
// rely on, and loses them silently. dialects.SQLite truncates 1.9 to 1 where all three
// dialects round it to 2; it answers 0 for CAST('abc' AS INTEGER) where
// dialects.PostgreSQL and dialects.GoogleSQL raise; it passes an invalid date straight through;
...
// original target type text so parameters survive. dialects.GoogleSQL's SAFE_CAST shares
...
// sqliteInteger is both a source-dialect type name and dialects.SQLite's own INTEGER
// cast target, which dialects.MySQL's DIV rewrite needs for its truncating division.
...
func castKindsFor(d dialects.Dialect) map[string]castKind {
	switch d {
	case dialects.MySQL:
...
// KnowsCastTarget reports whether the cast helpers convert to a type. A target
// they do not know is left to SQLite's own CAST, which is the only other
// conversion there is; sending it to a helper would answer NULL for a type the
// caller wrote.
...
func lookupCastKind(d dialects.Dialect, name string) (castKind, bool) {
...
// dialects.PostgreSQL and dialects.GoogleSQL raise, so the caller learns the data is bad. dialects.MySQL
// coerces instead: a non-numeric string becomes 0 (or its leading numeric
// prefix) and an invalid date becomes NULL, matching what dialects.MySQL itself returns
// alongside a warning.
func castValue(d dialects.Dialect, target string, v driver.Value) (driver.Value, error) {
	if v == nil {
		return nil, nil
	}
...
// fractional value on the way to an integer type, while dialects.SQLite truncates toward
// zero. dialects.PostgreSQL rounds halves to even; dialects.MySQL and dialects.GoogleSQL round them away
// from zero. A string is not a fractional value but a literal to be read, and
// the dialects part company there: dialects.PostgreSQL and dialects.GoogleSQL raise for text that
// is not an integer, and dialects.MySQL takes the digits at the front of it.
func castToInt(d dialects.Dialect, v driver.Value, strict bool) (driver.Value, error) {
	switch x := v.(type) {
	case int64:
		return x, nil
	case bool:
	...
	// dialects.GoogleSQL reads a hexadecimal string, which is how a column of
	// hexadecimal identifiers becomes numbers. The other two dialects do not:
	// dialects.PostgreSQL raises for it and dialects.MySQL reads the leading digits.
	if d == dialects.GoogleSQL {
	...
	if strict {
		// dialects.PostgreSQL and dialects.GoogleSQL read the string as an integer literal, so
		// "1.5" and "1e3" are not smaller integers but values the type cannot
		// hold: both engines raise there, and dialects.GoogleSQL's SAFE_CAST answers NULL.
	...
	// dialects.MySQL reads the leading run of digits and stops at the first character
...
// hexadecimalInt reads dialects.GoogleSQL's hexadecimal string form -- an optional sign
...
// mysqlIntegerPrefix returns the integer dialects.MySQL reads at the front of text when
...
// type, which is what dialects.MySQL answers alongside a warning.
...
//
// Every dialect here rounds a fractional value where dialects.SQLite truncates:
// dialects.PostgreSQL rounds halves to even, dialects.MySQL and dialects.GoogleSQL away from zero. Past the
...
// the range is checked here: dialects.MySQL clamps to the bound of the type, which is
...
func roundForDialect(d dialects.Dialect, f float64, strict bool) (driver.Value, error) {
	...
	case math.IsNaN(rounded):
		...
		// dialects.MySQL's answer for a value that names no number at all.
		return int64(0), nil
...
// dialects.MySQL clamps to the bound it passed, and the dialects that raise for a value
...
// a number, or 0 when there is none. It reproduces dialects.MySQL's coercion of a string
// to a number.
//
...
// string with no number in it at all — where dialects.MySQL answers 1.2. The same
// greedy scan stopped at the "e" of "1e5" and answered 1 for a number dialects.MySQL
...
// number need: dialects.MySQL's CAST('abc' AS TIME) and CAST('abc' AS YEAR) are both
...
func numericPrefixFound(s string) (float64, bool) {
	...
	// hands back the infinity along with the range error. dialects.MySQL answers the
...
// castToDecimal honors the scale that dialects.SQLite's REAL affinity discards.
// DECIMAL with no parameters means DECIMAL(10,0) in dialects.MySQL, so it rounds to a
// whole number there; dialects.PostgreSQL and dialects.GoogleSQL keep the full value.
func castToDecimal(d dialects.Dialect, v driver.Value, params []int, strict bool) (driver.Value, error) {
	...
	if len(params) >= 2 {
		scale = params[1]
	} else if len(params) == 1 || d == dialects.MySQL {
		scale = 0
	}
	if scale < 0 {
		return value, nil
	...
	// a value past it is clamped by dialects.MySQL and refused by the other two. Applying
...
// carrying no time of day at all: dialects.MySQL reads it as a number and spells it
// right to left as seconds, minutes and hours, so CAST('2024-03-05' AS TIME) is
// 00:20:24, while dialects.PostgreSQL refuses it. Formatting a date as a time answered
...
// mysqlTimeFromNumber is dialects.MySQL's reading of a value with no clock in it: the
...
// TIME) is 00:00:12. A magnitude past the TIME range answers NULL, as dialects.MySQL
// does.
func mysqlTimeFromNumber(v driver.Value) driver.Value {
	...
	if !found {
		// A value with no number in it is not a time at all, and dialects.MySQL answers
...
// castToYear implements dialects.MySQL's YEAR type: a date or datetime gives its year, a
...
// value outside all of that is not a year and answers NULL, which is what dialects.MySQL
...
// it cannot. dialects.MySQL stores 1901 through 2155 and nothing else.
...
// enforces by truncating and dialects.SQLite's TEXT affinity ignores.
...
// boolLiterals are the spellings dialects.PostgreSQL accepts for a boolean; dialects.GoogleSQL
// accepts the "true"/"false" subset. dialects.SQLite has no boolean type, so the result
...
func castToBool(_ dialects.Dialect, v driver.Value, strict bool) (driver.Value, error) {
	switch x := v.(type) {
	case bool:
		return boolToInt(x), nil
	case int64:
...
// unparseable string through unchanged, which is what dialects.SQLite's TEXT affinity
...
func castToTimeString(v driver.Value, layout string, strict bool) (driver.Value, error) {
	...
	// dialects.MySQL answers NULL for a value it cannot read as a date.
	return nil, nil
}
...
// dialects.GoogleSQL's SAFE_CAST.
func dialectCast(d dialects.Dialect, safe bool) scalarFn {
