...
// This file holds the GoogleSQL-only scalar functions: the ones with no SQLite
...
// googlesqlScalarFunctions are the deterministic GoogleSQL helpers.
func googlesqlScalarFunctions() map[string]scalarSpec {
	return map[string]scalarSpec{
		// BigQuery's digests answer bytes where PostgreSQL's MD5 and MySQL's
		...
		// than left to the name SQLite already has.
...
// fnToJSONString spells a value as JSON. SQLite has no boolean, so a boolean
...
func fnIEEEDivide(args []driver.Value) (driver.Value, error) {
	...
	out := a / b
	if math.IsNaN(out) {
		// SQLite has no NaN, and NULL is what this package answers in its
