...
func TestMemoryLimitsAndLargeFiles(t *testing.T) {
	...
	t.Run("Large single cell content", func(t *testing.T) {
		...
		db, err := OpenContext(ctx, tmpFile.Name())
		assert.NoError(t, err, "Failed to handle large cell content")
		defer db.Close()
		...
		assert.NoError(t, err, "Query failed on large cell")
		defer rows.Close()
		if err := rows.Err(); err != nil {
			assert.NoError(t, err, "Rows error on large cell")
			return
		}
		...
		if rows.Next() {
			if err := rows.Scan(&length); err != nil {
				...
				assert.Fail(t, "Expected cell length %d, got %d", cellSize, length)
			}
		}
	})
	...
	t.Run("Many rows stress test", func(t *testing.T) {
		...
		for i := range numRows {
			...
			if _, err := tmpFile.WriteString(line); err != nil {
				require.NoError(t, err, "Failed to write row %d", i)
			}
		}
		...
		db, err := OpenContext(ctx, tmpFile.Name())
		assert.NoError(t, err, "Failed to handle many rows")
		defer db.Close()
		...
		assert.NoError(t, err, "Count query failed")
		defer rows.Close()
		if err := rows.Err(); err != nil {
			assert.NoError(t, err, "Rows error on count query")
			return
		}
		...
		if rows.Next() {
			if err := rows.Scan(&count); err != nil {
				assert.NoError(t, err, "Count scan failed")
			} else if count != numRows {
				assert.Fail(t, "Expected %d rows, got %d", numRows, count)
			}
		}
	})
	...
	t.Run("Memory usage monitoring", func(t *testing.T) {
		...
		for i := range 10 {
			...
			if err != nil {
				cancel()
				assert.Fail(t, "Failed to open database %d: %v", i, err)
				continue
			}
			...
			if err != nil {
				_ = db.Close() // Ignore error in test cleanup
				cancel()
				assert.Fail(t, "Query failed for database %d: %v", i, err)
				continue
			}
			if err := rows.Err(); err != nil {
				...
				_ = db.Close()   // Ignore error in test cleanup
				cancel()
				assert.Fail(t, "Rows error for database %d: %v", i, err)
				continue
			}
		...
		if memoryIncrease > threshold {
			assert.LessOrEqual(t, memoryIncrease, threshold, "Memory usage increased by %d bytes (%.2f MB), may indicate memory leak (threshold: %.2f MB)",
	...
	t.Run("Chunk size configuration test", func(t *testing.T) {
		...
		for _, chunkSize := range chunkSizes {
			t.Run(fmt.Sprintf("ChunkSize_%d", chunkSize), func(t *testing.T) {
				...
				validatedBuilder, err := builder.Build(ctx)
				if err != nil {
					assert.Fail(t, "Build failed with chunk size %d: %v", chunkSize, err)
					return
				}

				db, err := validatedBuilder.Open(ctx)
				if err != nil {
					assert.Fail(t, "Open failed with chunk size %d: %v", chunkSize, err)
					return
				}
				defer db.Close()
				...
				rows, err := db.QueryContext(ctx, fmt.Sprintf("SELECT COUNT(*) FROM \"%s\"", tableName))
				if err != nil {
					assert.Fail(t, "Count query failed with chunk size %d: %v", chunkSize, err)
					return
				}
				defer rows.Close()
				if err := rows.Err(); err != nil {
					assert.Fail(t, "Rows error with chunk size %d: %v", chunkSize, err)
					return
				}
				...
				if rows.Next() {
					if err := rows.Scan(&count); err != nil {
						assert.Fail(t, "Count scan failed with chunk size %d: %v", chunkSize, err)
					} else if count != numRows {
						assert.Fail(t, "Expected %d rows with chunk size %d, got %d", numRows, chunkSize, count)
					}
				}
			})
		}
