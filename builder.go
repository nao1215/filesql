...
type DBBuilder struct {
	...
	// gate is the queue the returned handle's transactions and statements wait
	// in, shared by every connection of every handle this build hands out.
	gate *txGate
...
// session becomes a new sheet, unless its name is one a sheet already has that
// the sheet policy left out, which is refused rather than written over. A sheet
// the load read and no table remains for, which is what dropping or renaming a
// table leaves, refuses the save with ErrTableNotFound rather than keeping the
// rows it had beside the new sheet.
//
// A save of several sources produces all of them before it replaces any, so a
// save the writer refuses -- a tab in a TSV value, an LTSV table the session
// emptied -- leaves every source holding the rows it had rather than leaving the
// caller a directory holding two states of one session. What is left after that
// is the rename of each finished file over the source it replaces, which the
// operating system can still refuse: one refused after an earlier one has gone
// through leaves the sources before it replaced, and the error names the file it
// would not replace.
//
...
//     load's lock waits for it -- five seconds of waiting, half a minute in
//     all -- and then returns the error the database gave. A reader is the
//     exception: it cannot be read a second time, so only the step before the
...
func (b *DBBuilder) createInMemoryDatabase() (*sql.DB, error) {
	...
	b.gate = newTxGate()
	db := sql.OpenDB(&guardedConnector{drv: drv, dsn: dsn, gate: b.gate})
...
func (b *DBBuilder) setupDialectIfNeeded(ctx context.Context, db *sql.DB, readOnly bool) (*sql.DB, error) {
	...
	tdb := sql.OpenDB(&dialectConnector{drv: db.Driver(), dsn: b.handleDSN(readOnly), sqlDialect: b.sqlDialect, readOnly: readOnly, gate: b.gate})
...
func (b *DBBuilder) setupReadOnlyIfNeeded(ctx context.Context, db *sql.DB, readOnly bool) (*sql.DB, error) {
	...
	rodb := sql.OpenDB(&guardedConnector{drv: db.Driver(), dsn: b.handleDSN(true), readOnly: true, gate: b.gate})
