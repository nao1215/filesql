...
//     load's lock waits for it, for five seconds, and then returns the error
//     the database gave. A reader is the exception: it cannot be read a second
//     time, so only the step before the reading is tried again.
//
