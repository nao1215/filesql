...
2. **一貫性の維持**: AI生成コードがこのリポジトリの既存のコーディング規約と文書規約に従っていることを確認してください
3. **徹底的なテスト**: AI生成コードは全てのテストとリンティング（`make test`と`make lint`）に合格する必要があります
4. **プロジェクト設定の利用**: AIアシスタントに変更を依頼する前に、`go.mod`、`Makefile`、`.golangci.yml`、GitHub Actions など、このリポジトリを実際に規定している設定を確認してください
