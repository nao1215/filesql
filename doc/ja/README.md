...
**filesql** は、SQLite3のSQL構文を使用してCSV、TSV、LTSV、JSON、JSONL、Parquet、Excel (XLSX)ファイルを直接クエリできるGo SQLドライバーです。ファイルをメモリ上のSQLiteデータベースに自動で読み込むため、手動のインポート手順やスキーマ定義、データベースサーバーの起動なしに、ファイルに対してSQLを書けます。
...
- メモリ最適化のため`SetDefaultChunkSize()`でチャンクサイズ（チャンクあたりの行数）を設定
- すべてのデータはメモリ上のSQLiteデータベースに読み込まれるため、データセットのサイズにおおよそ比例したメモリを見込んでおく

#### メモリとストリーミング
filesqlは読み込み中にCSV、TSV、JSON配列をチャンク単位でストリーミングするため、パーサー自体がファイル全体を一度に保持することはありません。その他の形式は、レイアウト上の都合により読み込み時にすべてメモリへ読み込まれます:

- LTSV、配列ではないJSON/JSONLの値、Parquet（ランダムアクセスが必要）、Excel（XLSX、ZIPベース）は、読み込み前に全体を読み込みます。

いずれの場合も、パースされた行は最終的にメモリ上のSQLiteデータベースに格納されるため、合計のメモリ使用量はチャンクサイズだけでなくデータセットのサイズによって決まります。利用可能なメモリより大きなデータの場合は、ストリーミングだけに頼るのではなく、あらかじめファイルを分割するか、サブセットを読み込んでください。
...
### 並行処理
`Open`/`OpenContext`が返す`*sql.DB`は、複数のゴルーチン間で共有しても安全です。これは共有キャッシュ（shared-cache）のインメモリSQLiteデータベースに支えられているため、プールされた各コネクションは同じデータへの独自の接続をオープンし、`database/sql`がそれらを自動的に管理します。`SetMaxOpenConns(1)`を自分で呼び出す必要はありません：
...
SQLiteは共有インメモリデータベースへの書き込みを直列化するため、書き込みが多い並行処理では互いに待ち合いが発生しますが、読み取りは同時に進行できます。完全に独立したデータベースが必要な場合は、ゴルーチンごとに個別の`*sql.DB`をオープンしてください。

> `LoadInto`はこれとは異なります。`LoadInto`では自分自身の`*sql.DB`を渡すため、コネクションプールの設定はあなたの責任になります。素のインメモリデータベース（`sql.Open("sqlite", ":memory:")`）の場合、そのデータベースは単一の接続専用となるため、`db.SetMaxOpenConns(1)`を自分で呼び出してください。
