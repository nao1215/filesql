...
## Использование ИИ-ассистентов (LLM)
...
2. **Поддерживайте согласованность**: Убедитесь, что код, сгенерированный ИИ, следует существующим соглашениям этого репозитория по коду и документации
3. **Тщательно тестируйте**: Код, сгенерированный ИИ, должен пройти все тесты и линтинг (`make test` и `make lint`)
4. **Используйте конфигурацию проекта**: Перед тем как просить ИИ что-то менять, проверьте реальные файлы конфигурации репозитория, такие как `go.mod`, `Makefile`, `.golangci.yml` и GitHub Actions
