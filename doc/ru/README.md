...
### Многопоточность
`*sql.DB`, возвращаемый `Open`/`OpenContext`, безопасно использовать совместно из нескольких горутин. Он опирается на одно соединение SQLite в памяти, поэтому filesql ограничивает пул одним соединением (`SetMaxOpenConns(1)`), а `database/sql` сериализует доступ за вас:

```go
// Безопасно: совместное использование одного *sql.DB из нескольких горутин.
...
Обратите внимание, что поскольку доступ сериализуется через единственное соединение, запросы не выполняются параллельно; многопоточность безопасна, но не ускоряет работу. Если вам нужна настоящая параллельность, открывайте отдельный `*sql.DB` для каждой горутины.

> Когда вы используете `LoadInto` с собственным `*sql.DB`, за конфигурацию пула отвечаете вы сами. Для базы данных в памяти вызывайте `db.SetMaxOpenConns(1)` самостоятельно, потому что данные SQLite `:memory:` приватны для каждого соединения.
