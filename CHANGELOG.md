...
- `FORMAT('%t', x)` and `FORMAT('%T', x)` under the GoogleSQL dialect print the value ([#574](https://github.com/nao1215/filesql/issues/574)). `FORMAT` was rewritten to SQLite's printf, which covers the verbs the two share but knows neither of BigQuery's own, and printf answers NULL for a format string holding a verb it does not know -- so the whole call came back NULL, and the NULL travelled through whatever concatenated it. The call now goes to a helper that prints `%t` as the value's compact form and `%T` as the literal that would produce it, handing every other verb to the same formatting the printf path performed. A NULL prints as `NULL` for both verbs. A boolean reaches the helper as the integer SQLite stores it as, so it prints as 0 or 1 rather than as `true` or `false`, which is a limit of storing GoogleSQL's types in SQLite rather than of the verb.
