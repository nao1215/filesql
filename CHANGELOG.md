...
- README says what a file this package did not write can cost ([#526](https://github.com/nao1215/filesql/issues/526)). Three of the numbers a compressed or columnar file states about itself are now read and refused before the library that would allocate them sees the file — an xz stream's dictionary, a zstd frame's window, and a Parquet file's footer length — and one is not: a Parquet page header sits inside a column chunk and is decoded as the rows are read, and it states the size of the page's statistics, so a damaged 473-byte file allocates 98 MiB and then fails with `unexpected EOF`. The failure is fast, under a millisecond warm, so this is not the unbounded decode the Parquet reader was changed to avoid; what is out of proportion is the price, and it is paid on every call. Nothing in the reader can reach that number, and `parquet.OpenFile` takes no option that bounds what a decoded structure may declare, so what changes here is that a caller reading files from somewhere they do not control is told.
