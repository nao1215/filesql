...
- A saved REAL column loads as a REAL column ([#422](https://github.com/nao1215/filesql/issues/422)). A dump wrote a float in its shortest exact form, and the shortest form of a whole number carries no decimal point, so a column of `10.00` and `5.00` was written as `10` and `5` and reloaded as INTEGER: `SELECT amount/4` answered `2.5` before the save and `2` after it, because SQLite divides integers as integers. Which values were affected depended on their magnitude, since `1e6` renders as `1e+06` and reloads as REAL while `1000` renders bare. An infinity was lost outright, written as `+Inf`, which SQLite does not read as a number, so the reloaded column held the text of that word. Both now go through the rendering the Parquet import already used for the same reason: a whole number gains a `.0`, and an infinity is written as a literal that overflows to one. An INTEGER column is untouched, and a dump-and-load round trip is now checked as a property over two hundred generated tables built from the values type inference decides on, which is what caught this.
