...
- A workbook whose style table holds no date format is not asked about its cells ([#531](https://github.com/nao1215/filesql/issues/531)). Loading an 18.5 MB workbook of 200,000 rows cost 1849 MB of peak resident memory, against 101 MB for the same rows as a CSV, and reading the rows was not what cost it: opening the workbook is 24 MB and the streaming row read is 267 MB, while asking the library about one cell's style is 1470 MB, because that is when it builds the whole sheet as objects. `NormalizeXLSXDates` asked once per non-empty cell — 2.4 million times for that sheet — so the first question bought the entire cost. A cell is a date because of its style, so a workbook whose style table holds no date format has no date cells and there is nothing to ask about; the style table is a part of the file on its own and reading all of it costs nothing measurable. The same load is now 351 MB. What has not moved is a workbook that does have a date-formatted cell, which still costs 1939 MB, because deciding which cells wear that style is what the expensive question answers. Nothing about which cells become dates changed: the built-in and custom date formats, the colour brackets that are not elapsed units, the 1904 epoch and the serials below 61 all answer as they did.
